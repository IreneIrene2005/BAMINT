...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_payment') {
    ...
    // Validate input
    $errors = [];
    $bill_id_raw = isset($_POST['bill_id']) ? $_POST['bill_id'] : '';
    $bill_id = intval($bill_id_raw);
    
    if (empty($bill_id_raw)) $errors[] = "Please select a bill";
    ...
    // Handle different bill selection types
    if ($bill_id_raw === 'grand_total_due') {
        // Special case: Grand Total Due - allow this
        $bill = null;
    } elseif ($bill_id == 0 && isset($_GET['room_request_id'])) {
    ...
    if (!empty($errors)) {
        ...
        try {
            ...
            if ($bill_id_raw === 'grand_total_due') {
                // Grand Total Due: Create bills for all pending room requests that don't have bills yet
                $all_room_requests_stmt = $conn->prepare("
                    SELECT rr.* FROM room_requests rr
                    LEFT JOIN bills b ON rr.room_id = b.room_id AND rr.tenant_id = b.tenant_id AND b.notes LIKE '%ADVANCE PAYMENT%'
                    WHERE rr.tenant_id = :tenant_id AND rr.status = 'pending_payment' AND b.id IS NULL
                ");
                $all_room_requests_stmt->execute(['tenant_id' => $tenant_id]);
                $pending_requests = $all_room_requests_stmt->fetchAll(PDO::FETCH_ASSOC);
                
                $created_bills = [];
                foreach ($pending_requests as $rr) {
                    $room_id = $rr['room_id'];
                    ...
                    // Insert bill for this room request
                    ...
                    $bill_insert->execute([
                        ...
                        'due_date' => $due_date,
                        'notes' => 'ADVANCE PAYMENT for Room Request #' . $rr['id'],
                        'checkin_date' => $checkin ?: null,
                        'checkout_date' => $checkout ?: null
                    ]);
                    $created_bills[] = $conn->lastInsertId();
                }
                
                // Use the first created bill for payment processing, or find existing bills
                if (!empty($created_bills)) {
                    $bill_id = $created_bills[0];
                } else {
                    // Find existing bills for this tenant
                    $existing_bills_stmt = $conn->prepare("SELECT id FROM bills WHERE tenant_id = :tenant_id AND status IN ('pending', 'partial') ORDER BY id DESC LIMIT 1");
                    $existing_bills_stmt->execute(['tenant_id' => $tenant_id]);
                    $existing_bill = $existing_bills_stmt->fetch(PDO::FETCH_ASSOC);
                    if ($existing_bill) {
                        $bill_id = $existing_bill['id'];
                    } else {
                        throw new Exception("No bills found to process payment for.");
                    }
                }
            ...
            // Notify admins about new payment awaiting verification
            try {
                notifyAdminsNewPayment($conn, $bill_id, $tenant_id, $payment_amount);
            } catch (Exception $e) {
                error_log('notifyAdminsNewPayment failed: ' . $e->getMessage());
            }

            // Notify tenant that payment was submitted (confirmation)
            try {
                createNotification(
                    $conn,
                    'tenant',
                    ...
                    'payment_submitted',
                    'Payment Submitted',
                    'Your payment of ₱' . number_format($payment_amount, 2) . ' was submitted and is awaiting admin verification.',
                    $paymentId,
                    'payment_transaction',
                    'tenant_payments.php'
                );
            } catch (Exception $e) {
                error_log('tenant payment notification failed: ' . $e->getMessage());
            }

            // If this payment is for a room request (advance payment), notify admins about the room request as a booking
            if (isset($_GET['room_request_id']) && is_numeric($_GET['room_request_id'])) {
                $room_request_id = intval($_GET['room_request_id']);
                try {
                    $rr = $conn->prepare('SELECT tenant_count FROM room_requests WHERE id = :id');
                    $rr->execute(['id' => $room_request_id]);
                    $rrData = $rr->fetch(PDO::FETCH_ASSOC);
                    $tenantCount = $rrData ? intval($rrData['tenant_count']) : 1;
                    notifyAdminsNewRoomRequest($conn, $room_request_id, $tenant_id, $tenantCount);
                } catch (Exception $e) {
                                ...
                                <form method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control" id="bill_id" name="bill_id" required onchange="updateBillAmount()">
                                            <?php
                                            // Determine what to show in the bill selector.
                                            // If this page was opened for a `room_request_id`, prefer showing the advance (room-only) payment
                                            // until the payment is verified/approved (or bill is paid). After that, show the Grand Total (room + amenities).
                                            $show_advance_payment = false;
                                            if (isset($_GET['room_request_id']) && is_numeric($_GET['room_request_id'])) {
                                                $room_request_id = intval($_GET['room_request_id']);
                                                // Use the previously computed $pending_bills (from above) if available
                                                if (!empty($pending_bills)) {
                                                    foreach ($pending_bills as $pb) {
                                                        // If this is a virtual bill (id==0) or notes indicate advance payment, compute room-only amount
                                                        $is_advance = ($pb['id'] == 0) || (isset($pb['notes']) && stripos($pb['notes'], 'ADVANCE PAYMENT') !== false);
                                                        if ($is_advance) {
                                                            // Compute room total (nights * rate)
                                                            if ($pb['id'] == 0 && isset($pb['amount_due'])) {
                                                                $room_total = floatval($pb['amount_due']);
                                                            } else {
                                                                // Derive from room_requests and rooms table
                                                                $rrs = $conn->prepare("SELECT rr.checkin_date, rr.checkout_date, r.rate FROM room_requests rr JOIN rooms r ON rr.room_id = r.id WHERE rr.id = :rid LIMIT 1");
                                                                $rrs->execute(['rid' => $room_request_id]);
                                                                $rrrow = $rrs->fetch(PDO::FETCH_ASSOC);
                                                                $room_total = 0.0;
                                                                if ($rrrow && $rrrow['rate']) {
                                                                    $nights = 1;
                                                                    if (!empty($rrrow['checkin_date']) && !empty($rrrow['checkout_date'])) {
                                                                        $d1 = new DateTime($rrrow['checkin_date']);
                                                                        $d2 = new DateTime($rrrow['checkout_date']);
                                                                        $nights = max(1, (int)$d1->diff($d2)->format('%a'));
                                                                    }
                                                                    $room_total = floatval($rrrow['rate']) * $nights;
                                                                }
                                                            }
                                                            echo '<option value="' . intval($pb['id']) . '" data-amount="' . $room_total . '">';
                                                            echo 'Advance Payment (Room Only) - ₱' . number_format($room_total, 2);
                                                            echo '</option>';
                                                            $show_advance_payment = true;
                                                        } else {
                                                            // Non-advance bill option (fallback) - show bill's amount_due
                                                            $amount = isset($pb['amount_due']) ? floatval($pb['amount_due']) : 0.0;
                                                            echo '<option value="' . intval($pb['id']) . '" data-amount="' . $amount . '">';
                                                            echo 'Bill #' . intval($pb['id']) . ' - ₱' . number_format($amount, 2);
                                                            echo '</option>';
                                                        }
                                                    }
                                                } else {
                                                    // No pending advance bill found; calculate Grand Total from ALL room requests
                                                    // Sum of all room requests for this customer
                                                    ...
                                                    ");
                                                    $all_rooms_stmt->execute(['tenant_id' => $tenant_id]);
                                                    $all_room_requests = $all_rooms_stmt->fetchAll(PDO::FETCH_ASSOC);
                                                    
                                                    // Calculate total cost of all rooms booked
                                                    $total_rooms_cost = 0.0;
                                                    foreach ($all_room_requests as $rr) {
                                                        $rate = floatval($rr['rate']);
                                                        $nights = 1;
                                                        if ($rr['checkin_date'] && $rr['checkout_date']) {
                                                            $dt1 = new DateTime($rr['checkin_date']);
                                                            $dt2 = new DateTime($rr['checkout_date']);
                                                            $interval = $dt1->diff($dt2);
                                                            $nights = max(1, (int)$interval->format('%a'));
                                                        }
                                                        $total_rooms_cost += $rate * $nights;
                                                    }
                                                    ...
                                                    $additional_items_cost = 0.0;
                                                    foreach ($amenities as $a) {
                                                        $additional_items_cost += floatval($a['cost']);
                                                    }
                                                    
                                                    $grand_total_due = $total_rooms_cost + $additional_items_cost;
                                                    echo '<option value="grand_total_due" data-amount="' . $grand_total_due . '">';
                                                    echo 'Grand Total Due (All Rooms + Amenities) - ₱' . number_format($grand_total_due, 2);
                                                    echo '</option>';
                                                }
                                            } else {
                                                // Not a room_request flow: show Grand Total from ALL room requests
                                                // Sum of all room requests for this customer
                                                $all_rooms_stmt = $conn->prepare("
                                                    SELECT rr.*, r.rate
                                                    FROM room_requests rr
